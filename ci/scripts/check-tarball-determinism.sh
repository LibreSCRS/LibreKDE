...
# script makes is purely local; where a tree carries submodules or a dependency
# pinned for FetchContent, the same run reaches a third-party host twice per
# check, and a format-check that goes red because github.com is unreachable
# teaches people to ignore red. The property is measured wherever the clone is
# purely local, and stated as unmeasured in the release workflow of the rest.
#
